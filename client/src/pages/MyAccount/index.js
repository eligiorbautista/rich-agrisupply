...
const MyAccount = () => {
  ...
  const saveBillingAddress = async () => {
    ...
    setIsLoading(true);
    try {
      // Validate phone number first: required and at least 12 numeric characters
      const rawPhone = (billingAddress.phoneNumber || "").toString();
      const formattedPhone = rawPhone.replace(/\D/g, "");
      if (!formattedPhone || formattedPhone.length < 12) {
        // stop early and show a friendly error (don't proceed to call APIs)
        setIsLoading(false);
        context.setAlertBox({
          open: true,
          error: true,
          msg: "Phone number is required and must contain at least 12 digits",
        });
        return;
      }
      ...
      if (!userData) {
        ...
        // Update user data with billing address fields
        const updatedUserData = {
          ...userData,
          name: userData.name,
          email: userData.email,
          // save phone to user profile from billing address
      ...
      await editData(`/api/user/${user.userId}`, updatedUserData);
      // Update local UI and localStorage with new phone and billing info
        setFormFields(prev => ({ ...prev, phone: formattedPhone }));
      const storedUser = JSON.parse(localStorage.getItem('user')) || {};
        storedUser.phone = formattedPhone;
      storedUser.billingAddress = updatedUserData.billingAddress;
      ...
      context.setAlertBox({
        open: true,
        error: true,
        msg: error?.message || "Error saving billing address. Please try again.",
      });
    }
    setIsLoading(false);
  };
                ...
                <div className="col-md-12">
                  <div className="row">
                    ...
                    <div className="col-md-6">
                      <div className="form-group">
                        <TextField
                          label="Phone Number (required, exactly 11 digits)"
                          variant="outlined"
                          className="w-100"
                          name="phoneNumber"
