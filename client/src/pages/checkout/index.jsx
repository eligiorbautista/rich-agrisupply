...
const Checkout = () => {
  ...
  useEffect(() => {
    ...
    // Load user data for filling initial form values
    fetchDataFromApi(`/api/user/${user.userId}`).then((res) => {
      ...
      context.setAlertBox({
        open: true,
        error: true,
        msg: "Error loading user information"
      });
    });
  ...
  const payNow = (e) => {
    ...
    // Normalize phone and require exactly 11 digits
    const rawPhone = (formFields.phoneNumber || "").toString();
    const phoneDigits = rawPhone.replace(/\D/g, "");
    if (!phoneDigits || phoneDigits.length !== 11) {
      context.setAlertBox({
        open: true,
        error: true,
        msg: "Please enter a valid phone number (11 digits)",
      });
      return false;
    }
        ...
        <form className="checkoutForm" onSubmit={payNow}>
          <div className="row">
            <div className="col-md-8">
              ...
              <h6>Landmark (optional)</h6>

              <div className="row">
                <div className="col-md-12">
                  <div className="form-group">
                    <TextField
                      label="Cemetery / Hospital / School (optional)"
                      variant="outlined"
                      className="w-100"
                      size="small"
                      name="state"
