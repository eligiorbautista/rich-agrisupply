...
// Get billing address
router.get("/billing-address/:userId", async (req, res) => {
  try {
    console.log("Getting billing address for user:", req.params.userId);
    ...
    // Return empty billing address if none exists
    ...
    res.status(500).json({ success: false, message: "Error fetching billing address" });
  }
});

// Save/update billing address
router.post("/billing-address/:userId", async (req, res) => {
  try {
    console.log("Saving billing address for user:", req.params.userId);
    ...
    // Update billing address with provided fields
    user.billingAddress = {
    ...
    console.log("Saving user with updated billing address:", user.billingAddress);
    await user.save();
    ...
    res.status(500).json({ success: false, message: "Error saving billing address" });
  }
});
...
router.get("/billing-address/:userId", async (req, res) => {
  try {
    ...
    if (!user.billingAddress) {
      return res.status(404).json({ success: false, message: "No billing address found" });
    }
...
router.post("/billing-address/:userId", async (req, res) => {
  try {
    const { fullName, streetAddressLine1, streetAddressLine2, city, state, zipCode } = req.body;
...
router.put("/:id", async (req, res) => {
  ...
  const user = await User.findByIdAndUpdate(
    req.params.id,
    {
      ...
      password: newPassword,
      images: imagesArr,
      // include billingAddress if provided in the request body
      billingAddress: req.body.billingAddress || (userExist ? userExist.billingAddress : undefined),
    },
    { new: true }
  );
